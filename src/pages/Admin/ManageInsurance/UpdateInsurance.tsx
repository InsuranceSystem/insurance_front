    ...
    axios
      ...
      .then((response) => {
        ...
        if (response.data.data) {
          setName(response.data.data.insuranceName);
          setType(response.data.data.type);
          setMaxCompensation(
            Number(response.data.data.maxCompensation).toLocaleString()
          );
          setPeriodOfInsurance(response.data.data.periodOfInsurance);
          setAgeOfTarget(response.data.data.ageOfTarget);
          setBasicPremium(
            Number(response.data.data.basicPremium).toLocaleString()
          );
          setDistributionStatus(response.data.data.distributionStatus);
          setInsuranceClausePeriod(response.data.data.insuranceClausePeriod);
          setPrecaution(response.data.data.precaution);
          console.log(response.data.data.termsIDList);
          if (response.data.data && response.data.data.termsIDList) {
            const termsArray = response.data.data.termsIDList
              .split(',')
              .map((item: string) => item.trim());
            setTermsIdList(termsArray);
          } else {
            setTermsIdList([]);
          }
          console.log(termsIdList);
        } else {
          console.error('보험 데이터 존재 X', response);
        }
      })
      .catch((error) => {
